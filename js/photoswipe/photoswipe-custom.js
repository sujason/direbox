...
var publicMethods = {
	...
	goTo: function(index) {
		...
		// MOD: include index here
		_shout('beforeChangeAnim', _currentItemIndex);
	...
	updateCurrItem: function(beforeAnimation) {
		...
		// MOD: added this from below
		_shout('beforeChangeUpdate', self.currItem);
...
var _gestureStartTime,
	...
	_onDragStart = function(e) {
		...
		if(_preventDefaultEventBehaviour(e, true)) {
			...
			// MOD: instead skip gesture detection
			return;
		}
...
var _getItemAt,
	...
	_appendImage = function(index, item, baseDiv, img, preventAnimation, keepPlaceholder) {
		...
		if(img) {
			...
			// MOD: notify listeners when appending image
			_shout('appendImage', baseDiv, img, index)
...
_registerModule('Controller', {
	...
	publicMethods: {
		...
		setContent: function(holder, index) {
			...
			// MOD: notify listeners before adding
			_shout('setContent', index, holder, item);
			holder.el.innerHTML = '';
			...
			if(!_moved && !_isMultitouch && !_numAnimations) {
				...
				if(tapTimer) {
					...
					if ( _isNearbyPoints(p0, tapReleasePoint) ) {
						_shout('doubleTap', e, p0);
						// MOD: maintain zoom in when double tapping
						if (_currZoomLevel < 0.7) return;
					}
				}
				_equalizePoints(tapReleasePoint, p0);
				// MOD: moved this from bottom so that it's actually executed with mouse input
				tapTimer = setTimeout(function() {
					// MOD: only keep track of double tap time, don't register 2nd tap
			...
			if(_currZoomLevel <= self.currItem.fitRatio) {
				if( _options.modal ) {
					...
					if ( !_options.closeOnScroll ) {
						e.preventDefault();
						// MOD: scroll in overview changes images
						// if (e.deltaY > 0) { 
						// 		//self.goTo( _currentItemIndex + 1);
						// 		self.next();
						// } else {
